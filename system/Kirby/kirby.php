...
class core {
	...
	public static function msg($response) {
	...
	public static function trace($msg=false) {
...
class c {
	...
	public static function get_array($key, $default=null) {
	...
	public static function set_array($key, $value=null) {
		if (!is_array($value)) {
			$m = self::get_sub($key);
			foreach($m AS $k => $v) {
				self::set($k, $value);
...
class db {
	...
	public static function select($table, $select='*', $where=null, $order=null, $page=null, $limit=null, $fetch=true) {
	...
	public static function join($table_1, $table_2, $on, $select, $where=null, $order=null, $page=null, $limit=null, $type="JOIN") {
			return self::select(
	...
	public static function error($msg=null, $exit=false) {
...
class dir {
	...
	public static function size($path, $recusive=true, $nice=false) {
...
class l {
	...
	public static function change($language='en') {
...
class size {
	...
	public static function fit($width, $height, $box, $force=false) {
	...
	public static function fit_width($width, $height, $fit, $force=false) {
	...
	public static function fit_height($width, $height, $fit, $force=false) {
...
class str {
	...
	public static function email($email, $text=false) {
	...
	public static function link($link, $text=false) {
		$text = ($text) ? $text : $link;
	...
	public static function apostrophe($name) {
		return (substr($name,-1,1) == 's' || substr($name,-1,1) == 'z') ? $name .= "'" : $name .= "'s";
	}

	public static function plural($count, $many, $one, $zero = '') {
		if($count == 1) return $one;
		else if($count == 0 && !empty($zero)) return $zero;
		else return $many;
	}

	public static function substr($str,$start) {
	...
	public static function match($string, $preg, $get=false, $placeholder=false) {
...
class url {
	...
	public static function short($url, $chars=false, $base=false, $rep='…') {
...
class v {
	...
	public static function string($string, $options) {
		...
		if($format == 'alpha_numeric')
			if(!preg_match("/^([a-z0-9])+$/i", $string)) return false;
		if($format == 'alpha_dash')
			if(!preg_match("/^([-a-z0-9_-])+$/i", $string)) return false;
		if($format == 'alpha_dash_dot')
			if(!preg_match("/^([-a-z0-9._-])+$/i", $string)) return false;
		if($format == 'numeric')
			if(!preg_match( '/^[\-+]?[0-9]*\.?[0-9]+$/', $string)) return false;
		
		if($pattern && !preg_match('/^[$format]*$/is', $string)) return false;
